...
class AccountRepository: BaseRepository() {
    ...
    suspend fun login(username: String, password: String): BaseResponse<LoginRsp> {
        return apiService.loginAsync(username, password)
    }

    suspend fun register(username: String, password: String, rePassword: String): BaseResponse<LoginRsp> {
        return apiService.registerAsync(username, password, rePassword)
    }

    suspend fun getUserInfo() = apiService.getScoreInfoAsync()
}